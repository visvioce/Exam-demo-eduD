...
const cachedQuestions = ref<GeneratedQuestion[]>([])  // 后台生成完成的题目缓存
const isBackgroundGenerating = ref(false)  // 是否正在后台生成
...
async function handleAiGenerate() {
  ...
  if (isBackgroundGenerating.value) {
    ElMessage.info('AI正在后台生成中，请稍候...')
    return
  }
  
  // 如果有缓存的题目，先显示
  if (cachedQuestions.value.length > 0) {
    generatedQuestions.value = cachedQuestions.value
    cachedQuestions.value = []
    ElMessage.success(`加载了 ${generatedQuestions.value.length} 道之前生成的题目`)
  }
...
function handleCloseAiDialog() {
  ...
  if (generating.value && closeStream.value) {
    ...
    ElMessage.info('AI正在后台生成，完成后将通知您')
    aiDialogVisible.value = false
    return
  }
  // 正常关闭
      ...
      closeStream.value = aiApi.generateQuestionsStream(request, {
        ...
        onComplete: (response) => {
          ...
          if (isBackgroundGenerating.value) {
            ...
            cachedQuestions.value = response.questions
            ElNotification.success({
              title: 'AI生成完成',
              message: `已生成 ${response.questions.length} 道题目，点击AI出题可查看`,
              duration: 5000
            })
          } else {
