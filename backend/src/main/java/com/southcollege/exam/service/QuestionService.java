...
public class QuestionService extends ServiceImpl<QuestionMapper, Question> {
     ...
     * 检查题目是否可以删除
     * 被试卷引用的题目不能删除
     * 优化：使用数据库JSON函数，避免加载全表
     */
    public void checkCanDelete(Long questionId) {
        // 使用数据库JSON函数直接查询，避免N+1和全表加载
        List<String> paperNames = paperMapper.selectPaperNamesByQuestionId(questionId);
        if (!paperNames.isEmpty()) {
            throw new BusinessException("该题目已被试卷「" + String.join("」、「", paperNames) + "」引用，无法删除");
        }
    }
