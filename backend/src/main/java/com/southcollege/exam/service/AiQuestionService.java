...
public class AiQuestionService {
    ...
    private String buildPrompt(String subject, String type, String difficulty, Integer count, String requirements) {
        ...
        prompt.append("      ],\n");
        prompt.append("      \"correctAnswer\": \"正确答案（多选题为数组，其他类型为字符串）\",\n");
    ...
    private GeneratedQuestionResponse parseAiResponse(String aiResponse) {
        try {
            ...
            for (int i = 0; i < questionsArray.size(); i++) {
                ...
                item.setType(qObj.getStr("type"));
                item.setDifficulty(qObj.getStr("difficulty"));
                ...
                if (answerObj instanceof JSONArray) {
                    ...
                    item.setCorrectAnswer(JSONUtil.toJsonStr(answers));
                } else {
                    // 单选题/判断题/填空题/简答题 - 答案是字符串
                ...
                if (scoringArray != null) {
                    ...
                    for (int j = 0; j < scoringArray.size(); j++) {
                       JSONObject critObj = scoringArray.getJSONObject(j);
                        ...
                        criterion.setScore(critObj.getBigDecimal("score"));
    ...
    public SseEmitter generateQuestionsStream(Long userId, String subject, String type,
        ...
        executor.execute(() -> {
            try {
                ...
                if (config == null) {
                    // 发送错误事件而不是抛出异常
                    safeSendEvent(emitter, "error", "{\"message\": \"未配置AI或没有启用的AI配置，请先在个人中心配置AI\"}", isCompleted);
     ...
     */
    private void safeSendEvent(SseEmitter emitter, String eventName, String data, boolean[] isCompleted) {
        if (isCompleted[0]) {
     ...
     * 安全完成SSE连接
     */
    private void safeComplete(SseEmitter emitter, boolean[] isCompleted) {
        if (isCompleted[0]) return;
        isCompleted[0] = true;
        try {
            emitter.complete();
     ...
     * 安全地以错误完成SSE连接
     */
    private void safeCompleteWithError(SseEmitter emitter, Exception ex, boolean[] isCompleted) {
        if (isCompleted[0]) return;
        isCompleted[0] = true;
        try {
     ...
     * 流式调用AI API
     */
    private void streamCallAiApi(AiConfig config, String prompt, StreamCallback callback) {
