...
public class ExamController {
	...
	@Operation(summary = "获取已发布的考试列表", description = "学生可访问，返回已发布且未结束的考试")
	...
	@Operation(summary = "获取我的考试列表", description = "学生获取已加入课程的考试列表，附带考试状态")
    ...
    public Result<Boolean> save(
        ...
        exam.setTotalScore(paper.getTotalScore());  // 使用试卷总分，忽略前端传入的值
    ...
    public Result<Boolean> update(
        ...
        exam.setStatus(originalExam.getStatus()); // 状态必须通过 publishExam/cancelExam 修改

        // 已发布/进行中的考试不允许修改试卷、课程和时间等关键字段
        if (!ExamStatusEnum.DRAFT.getCode().equals(originalExam.getStatus())) {
            ...
            exam.setTotalScore(originalExam.getTotalScore());  // 已发布的考试保持原总分
        } else {
            // 草稿状态下，如果修改了试卷，同步更新总分
            if (examRequest.getPaperId() != null) {
                Paper paper = paperService.getById(examRequest.getPaperId());
                if (paper == null) {
                    throw new BusinessException("试卷不存在");
                }
                exam.setTotalScore(paper.getTotalScore());  // 同步试卷总分
            }
        }
    ...
    @RequireRole(RoleEnum.STUDENT)
    @Operation(summary = "提交考试", description = "学生提交答卷")
    ...
    @RequireRole(RoleEnum.STUDENT)
    @Operation(summary = "获取考试题目", description = "学生获取考试题目列表（不含正确答案）")
    ...
    @GetMapping("/{id}/paper")
    @Operation(summary = "获取考试试卷", description = "查看考试回顾时获取试卷信息")
    ...
    @Operation(summary = "获取考试回顾题目", description = "获取完整题目信息（含正确答案和解析）")
