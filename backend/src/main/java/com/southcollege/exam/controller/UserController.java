...
public class UserController {
    ...
    public Result<Boolean> delete(@Parameter(description = "用户ID") @PathVariable Long id) {
        // 检查用户是否有进行中的考试
        java.util.List<ExamSession> activeSessions = examSessionService.lambdaQuery()
                .eq(ExamSession::getStudentId, id)
                .eq(ExamSession::getStatus, "IN_PROGRESS")
                .list();
        if (!activeSessions.isEmpty()) {
            throw new com.southcollege.exam.exception.BusinessException("该用户有进行中的考试，无法删除");
        }
    ...
    public Result<Boolean> deleteBatch(@RequestBody List<Long> ids) {
        ...
        // 批量查询所有待删除用户的进行中考试，避免循环N+1
        java.util.List<ExamSession> activeSessions = examSessionService.lambdaQuery()
                .in(ExamSession::getStudentId, ids)
                .eq(ExamSession::getStatus, "IN_PROGRESS")
                .list();
        if (!activeSessions.isEmpty()) {
            java.util.Set<Long> activeUserIds = activeSessions.stream()
